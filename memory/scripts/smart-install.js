...
function isPluginDisabledInClaudeSettings() {
  try {
    ...
    return settings?.enabledPlugins?.['holyclaude'] === false;
  } catch {
    return false;
  }
}
 ...
 * 3. XDG path (~/.config/claude/plugins/holyclaude)
 * 4. Legacy path (~/.claude/plugins/holyclaude)
 */
function resolveRoot() {
  ...
  const pluginRel = join('plugins', 'holyclaude');
  const xdg = join(homedir(), '.config', 'claude', pluginRel);
  if (existsSync(join(xdg, 'package.json'))) return xdg;

  return join(homedir(), '.claude', pluginRel);
}
